...
# ── Per-sensor pipeline ───────────────────────────────────────────────────────
def run_sensor(client: bigquery.Client, sensor_name: str, location_id: int) -> pd.DataFrame | None:
    ...
    try:
        ...
        print(f"    [SKIP] Query gagal: {e}")
        return None

    if len(df) < 300:
        print(f"    [SKIP] Data terlalu sedikit: {len(df)} rows")
        return None
    ...
    if len(df_feat) < 200:
        print(f"    [SKIP] Setelah dropna terlalu sedikit: {len(df_feat)} rows")
        return None
    ...
    df_future = generate_future_features(df, hours=168)

    buffer = list(df['pedestrian_count'].tail(168).values)
    preds  = []
    for _, row in df_future.iterrows():
        row_feat = row[FEATURES].values.reshape(1, -1)
        pred = float(np.maximum(model.predict(row_feat), 0)[0])
        preds.append(pred)
        buffer.append(pred)
        buffer = buffer[-168:]
